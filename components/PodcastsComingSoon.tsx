...
export default function PodcastsComingSoon() {
	return (
		...
		>
			<h2 className="font-heading text-3xl text-hung mb-6">
			...
			<div className="grid gap-8 md:grid-cols-3">
				{podcasts.map((pod) => (
					<div
						key={pod.title}
						className="bg-black/80 rounded-xl border border-hung p-5 flex flex-col items-center"
					>
						<img
							src={resolvePublicImage(pod.image)}
							alt={pod.title}
							className="w-24 h-24 object-cover rounded-lg mb-3 border-2 border-hung"
						/>
						<div className="font-bold text-hung mb-1">{pod.title}</div>
						<p className="text-sm opacity-80 mb-1 text-center">
							{pod.desc}
						</p>
						<div className="text-xs text-hung">
