...
export default function UpcomingShows() {
  return (
    ...
    >
      <h2 className="font-heading text-3xl text-hotpink mb-6">Upcoming Shows</h2>
      <div className="grid gap-8 md:grid-cols-3">
        {shows.map((show) => (
          <div key={show.title} className="bg-black/80 rounded-xl border border-hotpink p-5 flex flex-col items-center">
            <img src={show.image} alt={show.title} className="w-24 h-24 object-cover rounded-lg mb-3 border-2 border-hotpink" />
            <div className="font-bold text-hotpink mb-1">{show.title}</div>
            <div className="text-xs mb-2">
              <span className="font-semibold">{show.date}</span> &bull; <span>{show.time}</span>
            </div>
            <div className="text-xs mb-1">Host: <span className="font-semibold">{show.host}</span></div>
            <p className="text-sm opacity-80 mb-1 text-center">{show.desc}</p>
          </div>
        ))}
      </div>
    </motion.section>
